...
import edu.sinica.iis.app.RoutingApplication.{hopper, routerManager, system}
...
object EvaluationEngine {
  def main(args: Array[String]): Unit = {
    ...
    val recorder = system.actorOf(RecorderActor.props("eval_car.json"))
    val routerManager = system.actorOf(RouterManager.props)

    val flavor = Flavor("cleanest", Parameters.Algorithms.ASTAR, true)
    ...
    val locationPairs = pointOfInterestsTest.combinations(2).toList
